...
static dma_addr_t mem_base = 0x88000000;
static dma_addr_t reg_base = 0x88001000;
...
static void plat_dummy_work(struct work_struct *work)
{
	...
	if (status & PLAT_IO_DATA_READY) {
		...
		queue_delayed_work(my_device->data_read_wq, &my_device->dwork,
				   my_device->js_pool_time);
}
...
static int plat_dummy_probe(struct platform_device *pdev)
{
	...
	pr_err("Memory mapped to %p\n", my_device->regs);
	pr_err("Registers mapped to %p\n", my_device->mem);
